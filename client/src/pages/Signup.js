...
const Signup = () => {
  ...
  const { signUp, signInWithGoogle, currentUser, userRole, needsRoleSelection } = useAuth();
  ...
  useEffect(() => {
    if (currentUser && userRole && !needsRoleSelection) {
      navigate(from);
    }
  }, [currentUser, userRole, needsRoleSelection, navigate, from]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Auth system will handle the rest (role selection, etc.)
    } catch (error) {
